...
/* Relativo al control de entrad y salida */
uint16_t aforo = 0;
uint8_t numMaxPaginas = 1;
identificacion Personas[5];
uint8_t paginaActual = 0;
...
extern int Init_ThRGB(void);
extern int Init_Temperatura(void);
extern int Init_Th_Sleep(void);
extern int Init_Th_Ricochet(void);
...
__NO_RETURN void app_main (void *arg) {
  (void)arg;
	...
	//Al iniciar la aplicacion contamos la cantidad de gente
  aforo = cuanta_gente();
